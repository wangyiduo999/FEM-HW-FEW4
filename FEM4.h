...
void FEM<dim>::define_boundary_conds() {
    ...
    for (int globalDOF = 0; globalDOF < totalNodes; globalDOF++) {

        // x1 = 0, u = 300
        if (nodeLocation[globalDOF][0] == 0.0) {
            boundary_values_of_D[globalDOF] = 300.0;
            boundary_values_of_V[globalDOF] = 0;
        } else if (nodeLocation[globalDOF][0] == 1.0) { //x1 = 1, u = 310
            boundary_values_of_D[globalDOF] = 310.0;
            boundary_values_of_V[globalDOF] = 0;
        }
    ...
    for (; elem != endc; ++elem) {
                ...
                for (unsigned int q = 0; q < num_quad_pts; q++) {
                    for (unsigned int i = 0; i < dim; i++) {
                        for (unsigned int j = 0; j < dim; j++) {
                            ...
                            Klocal[A][B] += kappa[i][j] * fe_values.shape_grad(A, q)[i] * fe_values.shape_grad(B, q)[j]
                                            * fe_values.JxW(q);
                        }
                    }
                }
        ...
        for (unsigned int i = 0; i < dofs_per_elem; ++i) {
            for (unsigned int j = 0; j < dofs_per_elem; ++j) {
                ...
                M.add(A_global, B_global, Mlocal[i][j]);
            }
        }
    }
}
...
void FEM<dim>::apply_initial_conditions() {
    ...
    for (unsigned int i = 0; i < totalNodes; i++) {
        if (nodeLocation[i][0] < 0.5) {
            ...
            D_trans[i] = 300.0 + 20 * (nodeLocation[i][0] - 0.5); //EDIT
        }
    }
